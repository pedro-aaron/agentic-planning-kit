...
$BlockStart = '# >>> agentic-planning-v3 (managed by install_kit.ps1) >>>'
$BlockEnd = '# <<< agentic-planning-v3 (managed by install_kit.ps1) <<<'
$LF = [string][char]10
...
if ($SkipCi) {
    ...
    if ((Test-Path -LiteralPath $workflowTarget) -and -not $Force) {
        ...
        $header = "# Generated by $Prefix/tools/install_kit.ps1. KIT_PATH points at the vendored kit."

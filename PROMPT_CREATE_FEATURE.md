# PROMPT_CREATE_FEATURE — Generate a lean feature plan under `.agentic_planning/`
...
- **Execution graph, human-readable.** `FEATURE_<SLUG>.md` §4 and `execution_prompts/README.md` show the dependency graph as a **Mermaid diagram** (parallel branches side by side) plus a `Depends on` column, and state in one line which steps may launch simultaneously and why that is safe (disjoint writes + disjoint exclusive resources, gates included). This *replaces* — never reintroduces — `execution-dag.json`.
...
- **Short handoff reports, nothing else.** Each step ends by writing `outputs/NN_<slug>.md` (≤40 lines: what changed, files touched, gate results — command + exit code —, tests added with a one-line claim each, test files touched, decisions taken, anything the next step must know). No `tests/`, `evals/`, `fixes/`, cache or cycle directories, no manifests, no scorecards.
- **Map-sync only when structure changed.** If the feature adds/moves a seam, recipe, blessed library, store protocol, subproject **or quality gate**, the last step updates the touched `WORKSPACE_MAP.md` sections in place (and entry-point pointer blocks for a new subproject). If nothing structural changed, there is no map-sync step at all.
- **Feature index registration — keep it hydrated.** `.agentic_planning/README.md` is the single-writer **feature index**: one row per feature (what it does, impacted subprojects, status), **ordered descending (newest first)**. Statuses: `📝 Diseñada` · `🔄 En ejecución` · `✅ Ejecutada` · `⛔ Superseded`. Planning a feature **adds its row at the top** (status `📝 Diseñada`; create the file from the index template below if missing). The **final step of every plan** — the map-sync step when present, otherwise the last step — updates the row to `✅ Ejecutada (YYYY-MM-DD)` as part of its declared deliverables. If the new feature supersedes an older one, flip the old row to `⛔ Superseded` in the same edit. No plan ships without its row; no finished feature leaves its row stale.
...
3. **Write `FEATURE_<SLUG>.md`** using the template below. §3 (contract) is the spine — be concrete: the binding test cases of every code step derive from it.
4. **Decompose into 2–6 single-session steps and assign dependencies for maximum safe parallelism**: check each pair of candidate-parallel steps for overlapping write scopes and shared exclusive resources (map concurrency tables — **including their gate commands**) before leaving them unconnected. For each code step, derive its binding test cases from §3 (happy path, negative, edge). Write each step file with the template below.
5. **Write `execution_prompts/TRIGGERS.md`** — one launcher block per step, in dependency order — and `execution_prompts/README.md` — the index table + Mermaid execution graph + the one-line parallel-safety note (state gate-run staggering when two parallel steps share an exclusive gate resource).
6. **Register the feature in `.agentic_planning/README.md`** (the feature index): insert its row **at the top** with status `📝 Diseñada`, the one-line description and the impacted subprojects; create the file from the index template if it doesn't exist; flip any superseded feature's row in the same edit. Make sure the plan's final step lists the `✅ Ejecutada` flip among its deliverables.
...
**Execution plan:** [`execution_prompts/README.md`](./execution_prompts/README.md)
...
| # | Step | Subproject | Depends on | Suggested effort | Gates | Writes |
|---|------|-----------|------------|------------------|-------|--------|
| 01 | ... | ... | — | medium | <map gates or `n/a (no product code)`> | ... |
...
## Template — a step file `NN-<slug>.md`
...
- <final step only> update the feature's row in `.agentic_planning/README.md` → `✅ Ejecutada (YYYY-MM-DD)`.
````

## Template — `execution_prompts/README.md`

````markdown
# Execution prompts — feature "<title>"

Launch each step in a fresh agent session via its trigger in [`TRIGGERS.md`](./TRIGGERS.md), **respecting the dependency edges below. Steps with no edge between them may run in parallel** (their write scopes and exclusive resources — gate commands included — are disjoint by construction — see the safety note under the graph). Code steps pass their subproject's quality gates before finishing. After the last step, the user runs the manual QA checklist in [`../FEATURE_<SLUG>.md`](../FEATURE_<SLUG>.md) §7.

| # | Step | Subproject | Depends on | Suggested effort | Gates | Report |
|---|------|-----------|------------|------------------|-------|--------|
| 01 | <title> | ... | — | medium | <gates or n/a> | `outputs/01_<slug>.md` |
...
<parallel-safety note: which steps may launch simultaneously and why; state gate-run staggering when two parallel steps share an exclusive gate resource.>
````

## Template — `execution_prompts/TRIGGERS.md`
...
```text
Read .agentic_planning/_feature_<slug>/execution_prompts/01-<slug>.md and execute it as your complete task spec.
...
## Template — `.agentic_planning/README.md` (feature index)

Create only if missing; otherwise **edit rows in place** (insert new features at the top of the table, flip statuses, never reorder the rest). User-facing prose in Spanish.

````markdown
# Índice de features — `.agentic_planning/`

Índice canónico de los planes de este workspace, **ordenado descendente** (lo más reciente arriba). Single-writer y siempre hidratado: crear un plan agrega su fila arriba; el último paso de cada plan cambia su fila a ✅ Ejecutada; un feature que supersede a otro cambia también la fila del viejo.

**Status:** ✅ Ejecutada · 🔄 En ejecución · 📝 Diseñada (pendiente de ejecución) · ⛔ Superseded · 📌 Documento vivo.
**Impacta:** <leyenda de subproyectos del workspace, desde WORKSPACE_MAP.md>.

## Features

| Feature | Qué hace | Impacta | Status |
|---|---|---|---|

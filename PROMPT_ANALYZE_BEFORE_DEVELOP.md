...
```text
.agentic_planning/_analysis_<slug>/ANALYSIS_<SLUG>.md
```
...
The only authorized writes are **two**: the new report (plus any parent directory needed for it) and **its single row in the `.agentic_planning/README.md` index** — see "Analysis index registration" below. Nothing else.
...
5. When the target is under VCS, record the analyzed HEAD/revision and whether the worktree is clean or dirty; do not disturb unrelated user changes.
...
2. **Question, scope, and assumptions** — mode, in/out of scope, analyzed revision/worktree state, and assumptions.
...
`.agentic_planning/README.md` is the single-writer index of this workspace's plans **and analyses**. An analysis that is not in the index is invisible to the next session, which is how the same question gets investigated twice. Registering is therefore part of the deliverable, not an optional courtesy.

After writing the report, add **one row at the top** of the index's `## Análisis` table (newest first). Create the section from the template below if it does not exist yet; if the index file itself is missing, create it from the template in `PROMPT_CREATE_FEATURE.md` and then add the section.
...
- **Write only your own row.** Never rewrite, reorder, or restate other rows, and never touch the `## Features` table — a different prompt owns it.
...
Add the matching legend line to the index header when creating the section:
...
- the analysis is registered with exactly one new row at the top of the `## Análisis` table in `.agentic_planning/README.md`, with no other row modified.
